...
export const ActivityLogPanel: React.FC = () => {
  const { activityLogs, clearActivityLogs, deviceId } = useEnergyData();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800/80">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">
              Real-time telemetry event records & remote load state transitions
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2 font-mono">
          <button
            ...
            className="flex items-center space-x-1.5 rounded-lg bg-slate-950 px-3 py-1.5 text-xs font-bold text-slate-300 border border-slate-800 hover:bg-slate-800 hover:text-white transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center space-x-1.5 rounded-lg bg-slate-950 px-3 py-1.5 text-xs font-bold text-rose-400 border border-slate-800 hover:bg-rose-950/40 transition-colors"
          >
      ...
      <div className="max-h-72 overflow-y-auto space-y-2 pr-1">
        {filteredLogs.length === 0 ? (
          ...
          filteredLogs.map((log) => (
            <div
              key={log.id}
              className="flex items-start justify-between rounded-xl bg-slate-950/80 p-3 border border-slate-800/80 hover:border-slate-700 transition-colors font-mono"
            >
              <div className="flex items-start space-x-3">
                ...
                <div className="space-y-0.5">
                  <div className="flex items-center space-x-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-[9px] uppercase font-bold border ${getLogBadge(
                        log.type
                      )}`}
                    >
                      {log.type}
              ...
              <div className="text-right text-[10px] text-slate-500 space-y-0.5 flex-shrink-0 ml-3">
